...
(function() {
  ...
  var root$2 = /* @__PURE__ */ from_html(`<div class="metadata-container svelte-qx734j"><div class="metadata-header svelte-qx734j"><h1 class="svelte-qx734j">Metadaten-Verwaltung</h1> <div class="header-actions svelte-qx734j"><button class="bright">Neues Schema</button></div></div> <!> <!> <div class="metadata-content svelte-qx734j"><!></div></div>`);
  ...
  const _RelationshipGraphService = class _RelationshipGraphService {
    constructor(document2, persistence, foundryAdapter2) {
  ...
  const _NotificationService = class _NotificationService {
    constructor(foundryAdapter2, logger) {
      this.foundryAdapter = foundryAdapter2;
      this.logger = logger;
    }
    showSuccess(message) {
  ...
  class FoundryLogger extends BaseService {
    constructor(foundryAdapter2) {
      super();
      this.foundryAdapter = foundryAdapter2;
      this.API_NAME = "logger";
      this.SERVICE_TYPE = "singleton";
    }
    info(message) {
  ...
  const _ConsoleErrorHandler = class _ConsoleErrorHandler {
    constructor(logger, foundryAdapter2) {
      this.logger = logger;
      this.foundryAdapter = foundryAdapter2;
    }
  ...
  const _ModuleInitializer = class _ModuleInitializer {
    constructor(logger, errorHandler, registrationService) {
      this.logger = logger;
  ...
  class ServiceFactory {
    ...
    initializeCrossCuttingServices() {
      const foundryAdapter2 = new FoundryAdapter();
      this.crossCuttingServices.set(SERVICE_IDENTIFIERS.FOUNDRY_ADAPTER, foundryAdapter2);
      const logger = new FoundryLogger(foundryAdapter2);
      this.crossCuttingServices.set(SERVICE_IDENTIFIERS.FOUNDRY_LOGGER, logger);
      const errorHandler = new ConsoleErrorHandler(logger, foundryAdapter2);
    ...
    registerAllServicesInAPI() {
      ...
      for (const [identifier, ServiceConstructor] of this.serviceRegistry) {
        if (identifier === SERVICE_IDENTIFIERS.FOUNDRY_LOGGER || identifier === SERVICE_IDENTIFIERS.CONSOLE_ERROR_HANDLER) {
          continue;
        }
      ...
      logger.info(`✅ Relationship App: ${this.serviceRegistry.size} services registered in API`);
    }
  ...
  foundryAdapter.onReady(async () => {
    const { serviceManager } = globalThis.relationshipApp;
    try {
      ...
      const foundryAdapter2 = serviceManager.resolve(SERVICE_IDENTIFIERS.FOUNDRY_ADAPTER);
      foundryAdapter2.showError("Relationship App initialization failed. Check console for details.");
    }
  });
})();
